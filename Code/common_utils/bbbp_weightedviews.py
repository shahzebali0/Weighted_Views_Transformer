...
def structuretoviews(pending,viewlength= 229,weight=1.,done=None,oxyz=(False,False,False,False),tol=1e-14,carbonbased= False, not_hydrogen= False, heaviest_origin= False):
...
def bbbp_filetowvmats(alist, speciesmap, carbonbased= False, not_hydrogen= False, heaviest_origin= False, viewlength=229):
    """
    Convert a list of atoms to np.arrays of weights and views for the bbbp dataset.
    ...
    :rtype: np.array, np.array
    """
    wvlist = structuretoviews(alist, viewlength=229, carbonbased=carbonbased, not_hydrogen=not_hydrogen , heaviest_origin=heaviest_origin)
    ## not to include 
    w, v = matricizeweightsviews(wvlist, speciesmap)
    return w, v
###

#from bbbp import molist

def load_bbbp_data(mollist, speciesmap, setNatoms=229, setNviews=229, carbonbased= False, not_hydrogen= False, heaviest_origin= False,verbose=0):
    species = set()
    ...
    varrays = []
    for mol in mollist:
        w, v = bbbp_filetowvmats(mol, speciesmap, carbonbased, not_hydrogen, heaviest_origin, Natoms)
        warrays.append(w)
        varrays.append(v)
