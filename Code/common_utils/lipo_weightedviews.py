...
speciesdict = {
    ...
    'N':(2,15), 'O':(2,16), 'F':(2,17),

    # Period 3
    'Na':(3,1), 'Mg':(3,2), 'Al':(3,13), 'Si':(3,14),
    'P':(3,15), 'S':(3,16), 'Cl':(3,17),

    # Period 4
    'K':(4,1), 'Ca':(4,2), 'Ti':(4,4), 'V':(4,5),
    'Cr':(4,6), 'Mn':(4,7), 'Fe':(4,8), 'Co':(4,9),
    'Ni':(4,10), 'Cu':(4,11), 'Zn':(4,12),
    'Ge':(4,14), 'As':(4,15), 'Se':(4,16), 'Br':(4,17),

    # Period 5
    'Rb':(5,1), 'Sr':(5,2), 'Y':(5,3), 'Zr':(5,4),
    'Nb':(5,5), 'Mo':(5,6), 'Ru':(5,8), 'Rh':(5,9),
    'Pd':(5,10), 'Ag':(5,11), 'Cd':(5,12),
    'In':(5,13), 'Sn':(5,14), 'Sb':(5,15),
    'Te':(5,16), 'I':(5,17),

    # Period 6
    'Cs':(6,1), 'Ba':(6,2),
    'La':(6,3), 'Ce':(6,3), 'Pr':(6,3),
    'Nd':(6,3), 'Sm':(6,3), 'Gd':(6,3),
    'Dy':(6,3), 'Lu':(6,3),
    'Hf':(6,4), 'Ta':(6,5), 'W':(6,6),
    'Re':(6,7), 'Ir':(6,9), 'Pt':(6,10),
    'Au':(6,11), 'Hg':(6,12), 'Pb':(6,14),
    'Bi':(6,15)
}
def speciesmap(s):
...
def structuretoviews(pending,viewlength= 203,weight=1.,done=None,oxyz=(False,False,False,False),tol=1e-14,carbonbased= False, not_hydrogen= False, heaviest_origin= False):
...
def lipo_filetowvmats(alist, speciesmap, carbonbased= False, not_hydrogen= False, heaviest_origin= False, viewlength=203):
    """
    Convert a list of atoms to np.arrays of weights and views for the lipo dataset.
    ...
    :rtype: np.array, np.array
    """
    wvlist = structuretoviews(alist, viewlength=203, carbonbased=carbonbased, not_hydrogen=not_hydrogen , heaviest_origin=heaviest_origin)
    ## not to include 
    w, v = matricizeweightsviews(wvlist, speciesmap)
    return w, v
###

#from lipo import molist

def load_lipo_data(mollist, speciesmap, setNatoms=None, setNviews=None, carbonbased= False, not_hydrogen= False, heaviest_origin= False,verbose=0):
    species = set()
    ...
    varrays = []
    for mol in mollist:
        w, v = lipo_filetowvmats(mol, speciesmap, carbonbased, not_hydrogen, heaviest_origin, Natoms)
        warrays.append(w)
        varrays.append(v)
