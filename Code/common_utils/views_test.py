...
def filetowvmats(alist, speciesmap, carbonbased= False, not_hydrogen= False, heaviest_origin= True, viewlength=None):
    """
    Convert a list of atoms to np.arrays of weights and views.
    ...
    :rtype: np.array, np.array
    """
    wvlist = structuretoviews(alist, viewlength=viewlength, carbonbased=carbonbased, not_hydrogen=not_hydrogen , heaviest_origin=heaviest_origin)
    ## not to include 
...
def load_chiral_data(test_mollist, speciesmap, setNatoms=29, setNviews=29, carbonbased= False, not_hydrogen= False, heaviest_origin= False,verbose=0):
    species = set()
    Natomslist = [len(mol) for mol in test_mollist]
    for mol in test_mollist:
    ...
    if verbose:
        print(f"{len(test_mollist)} molecules will be processed.")
    ...
    varrays = []
    for mol in test_mollist:
        w, v = filetowvmats(mol, speciesmap, carbonbased, not_hydrogen, heaviest_origin, Natoms)
        warrays.append(w)
        varrays.append(v)
