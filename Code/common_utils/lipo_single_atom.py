...
# ============================================================
# 1. Split each molecular view into individual atoms
# ============================================================

def small_views(vs, piece_size=4):
    """
    Split views so each atom contains:
    [atomic_number, x, y, z]
    """

    if vs.shape[-1] % piece_size != 0:
        raise ValueError(
            f"View size {vs.shape[-1]} is not divisible "
            f"by piece size {piece_size}."
        )
...
atom_properties = {
    ...
    "N": {
        ...
        "group_number": 15,
        "covalent_radius": 71,
        "first_ionization_energy": 14.534,
    ...
    "O": {
        ...
        "group_number": 16,
        "covalent_radius": 66,
        "first_ionization_energy": 13.618,
    ...
    "F": {
        ...
        "group_number": 17,
        "covalent_radius": 57,
        "first_ionization_energy": 17.423,
    ...
    "P": {
        ...
        "group_number": 15,
        "covalent_radius": 107,
        "first_ionization_energy": 10.487,
        "electron_affinity": 0.745,
        "atomic_volume": 17.0
    },
    ...
    "S": {
        ...
        "group_number": 16,
        "covalent_radius": 105,
        "first_ionization_energy": 10.360,
    ...
    "Br": {
        ...
        "group_number": 17,
        "covalent_radius": 120,
        "first_ionization_energy": 11.814,
    ...
    "I": {
        ...
        "group_number": 17,
        "covalent_radius": 139,
        "first_ionization_energy": 10.451,
...
def get_embeddings(
    ...
    """
    Create fixed-size atomic embeddings containing:
    ...
    property_keys = [
        prop
        for prop, is_on in single_atomic_property_switches.items()
        if is_on
    ]

    # 1 atomic number + 3 coordinates + selected properties
    num_features = 1 + 3 + len(property_keys)

    if num_features > embedding_size:
        raise ValueError(
            f"Embedding size {embedding_size} is too small "
            f"for selected features (requires at least {num_features})."
        )
        ...
        for view_id in range(number_of_views):
            ...
            for atom_id in range(number_of_atoms):
                ...
                if atomic_number == 0.0:
                    ...
                    atom_type = atomic_number_to_type.get(
                        int(atomic_number)
                    )

                    if atom_type is None:
                        raise ValueError(
                            f"Unknown atomic number: {atomic_number}"
                        )

                    properties = [
                        atom_properties[atom_type][prop]
            ...
            molecule_extended.append(
                np.array(view_extended)
            )

        atom_embeddings.append(
            np.array(molecule_extended)
        )
